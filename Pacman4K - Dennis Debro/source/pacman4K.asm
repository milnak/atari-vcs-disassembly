...
; individual that liked Pac-man and wanted to see a faithful version created for
; the Atari2600.
;
; Hopefully you will have as much fun playing it or learning from the source as 
; I had creating this game.
;
; Pacman4K source code and assembled binaries have been released for educational
; purposes only. It cannot be sold or made into cartridges without authorization
; from the author. If you find something useful in the code that you would like 
; to use, all I ask is that you give credit from the source you pulled it from.
; ==============================================================================
;
; Pacman4K started out as a challenge for me to see if I could get a no frills 
; Pac-man written for the 2600 in 4K and stay faithful to the original arcade 
; game. I made some sacrifices along the way. Most noticeable was the decision 
; to flicker the objects at a rate of 20Hz instead of developing a variable 
; flicker algorithm. I did some work in this direction but found that it took up
; too much ROM to include the other features.
;
...
; I commend them for their work as this was a basis for my maze layout. They 
; were able to do their game without using any undocumented 6502 opcodes. I used
; undocumented opcodes to reduce ROM and to help in kernel timing. Hats off to 
; you Ebivision!
;
; I also had some help in the testing department thanks to John Champeau, Robert
; DeCrescenzo, and Tony Wong. I would send ROMs to them as this project was
; nearing completion. They were kind enough to give me their feedback and report
; bugs I missed.
; Robert also sent me sound data from his 7800 Pac-man collection to help me in 
; the TIA department. If you have an Atari7800 and would like a Pac-man game for
; it then visit the AtariAge store at
; http://www.atariage.com/store/index.php?main_page=product_info&products_id=849
;
; The monster and Pac-man graphics were supplied by Stefan Haddewig (Retrofan) 
; of AtariAge. I had originally gone with monster graphics that were supplied by
; Clay Halliwell (ZylonBane) of AtariAge. Clay's graphics were more of a cartoon
; type of look. A poll was conducted on AtariAge and the members of the message 
; board voted that they preferred the A8/5200 look of the monsters so I changed 
; them.
;
; The game siren was taken from "A Better Pac-man" by Rob Kudla. Check out his 
; hack at http://www.kudla.org/raindog/pac26.html. I checked with Rob to see if 
; it was okay for me to use his data and he was fine with it. Thanks Rob, I 
; think your siren sounds are perfect.
;
; I released a number of demos on the AtariAge forum. One ask I received was to 
; speed up the movement. The movement calculations were based on the frame delay
; values I had received from Eduardo Mello and doing frame delay calculations 
; based on the 7800 version of Ms. Pac-man. I prefered the original movement 
; values but increased them to satsify the players on AtariAge. You can
; manipulate the speed of your assembly by setting the FASTER_SPEED assembler 
; switch below. Setting this to TRUE will increase the original speeds by 
; roughly 14%.
;
; I originally didn't have enough ROM to implement the speed differences using
; the difficulty switches. It may be possible now that ROM has been reclaimed 
; with version 1.2
;
; ==============================================================================
; Version 1.1 Notes
;
; Minor changes with this version. A compiler switch was added to build an 
; AtariAge exclusive edition. The original flagship was modified to be the 
; AtariAge logo. The startup tune was added too.
;
; I had e-mail conversations is Kurt Howe (Nukey Shay) of AtariAge on how to
; squeeze in the bytes. He convinced me to use the LSB data of the PF0 graphic 
; data to hold the bytes. I then added a baseline to the routine. I'm not too 
; good in the TIA sound department but I think it turned out well.
;
; Unfortunately the sound is dependent on the frame count. This will cause the 
; startup tune to be slower on PAL50 configurations than on the NTSC
; configuration.
;
; ==============================================================================
; Version 1.2 Notes
;
; Thomas Jentzsch took a look at the source and pointed out more byte savings.
; He also had a look at the monster AI. The monster AI has been modified to 
; better mimic the arcade game.
;
; At the time I completed the first version the Pac-man Dossier hadn't been 
; created or I didn't know about it. This can be found at
; http://home.comcast.net/~jpittman2/pacman/pacmandossier.html. This is an
; excellent write up of Pac-man and how the monsters behave.
;
; My first attempt was close however Thomas used the Pac-man Dossier to have 
; monsters target tiles like the arcade instead of the pixels I originally had.
; Now the game feels right. Thanks Thomas!
;
; The source code was also cleaned up for release to the public. Hopefully 
; everything is commented to the point that the source is easy to understand.
;
; I also found three bugs in the first version while working in Thomas'
; additions. They are...
;     (1) The SELF published version would show a garbled graphic during Pac-man
;        death
;        - This wasn't seen in the AtariAge released version so no carts were
;        affected
;        - It was caused by the graphics sitting too high on a page
;     (2) Startup tune could play when player RESET game after eating a bonus 
;        fruit
;        - This would happen if the player ate a bonus fruit and RESET the game 
;        before touching a monster
...
;        - This happens if Pac-man collided with a monster while the fruit sound
;         was still playing
;
; ==============================================================================
; Version 1.3 Notes
;
; Slight modification to the source to allow the game to play on the 2016
; edition of AtGames' Atari Flashback Portable.
;
; The original source would hit HMOVE at cycle 74 to eliminate the HMOVE lines
; from the display. I orginally did this because I had plans to implement a
; variable flicker routine. There was no need for this since the objects after
; the score is displayed and the background color of the screen is black.
;
; I modified the horizontal pixel test conditions by subtracting 8 from their
; original values to compensate for the HMOVE strobe now happening at the
; standard cycle 0.
;
; ==============================================================================
; Version 1.3.1 Notes
;
; Modified code to look for definition of constant assemble switch values so
; it can easily be assembled using a MAKEFILE
;
; I saw an opportunity to save 2 bytes in ROM.
;
; When looking through the source I noticed I was setting the y register twice
; for the monster animations. I saved 2 bytes by removed the extra one from
; .setMonsterVerticalAnimation.
;
; ==============================================================================
; Version 1.3.2 Notes
;
; The original game selection routine was reduced by half!
;
; Prior I was doing a compare to see if the selection had passed the maximum
; selected level. Since the maximum selection is 7; I could instead AND #7 to
...
PUBLISHER               = SELF      ; set the publisher of the game

   ENDIF

   IF !(PUBLISHER = ATARIAGE || PUBLISHER = SELF)

      echo ""
      echo "*** ERROR: Invalid PUBLISHER value"
      echo "*** Valid values: ATARIAGE = 0, SELF = 1"
      echo ""
      err
...
FASTER_SPEED            = FALSE     ; set to FALSE for original speed
   ...
   IF !(FASTER_SPEED = TRUE || FASTER_SPEED = FALSE)

      echo ""
      echo "*** ERROR: Invalid FASTER_SPEED value"
      echo "*** Valid values: FALSE = 0, TRUE = 1"
      echo ""
      err
...
PACMAN_COLOR            = YELLOW + 11; D0 will pause Pac-man for eating monster
MAZE_COLOR              = DK_BLUE + 4
...
LTBLUE_COLOR            = ULTRAMARINE_BLUE + 8

   ELSE

PEACH_COLOR             = BRICK_RED + 10
PINKY_COLOR             = RED + 10
...
LTBLUE_COLOR            = BLUE + 10

   ENDIF

   IF PUBLISHER = ATARIAGE

FLAGSHIP_COLOR          = LTBLUE_COLOR
...
PINKY_INTERESTED_OFFSET_VALUE = SIZE_TILE * 4; Pinky looks 4 tiles ahead
INKY_INTERESTED_OFFSET_VALUE = SIZE_TILE * 2; Inky looks 2 tiles ahead
...
INKY_START_X            = BLINKY_START_X - 11
...
;===============================================================================
   ...
   .org $80
;
; There are only 128 bytes of RAM available to the programmer.
...
MazeData
;
; This data is 255 bytes long so it must start at the beginning of a page 
; boundary. Placing it at the top of ROM guarantees it starts on a page.
;
; I could have used less bytes for the maze layout if I chose to reduce the
; resolution. I prefered the look of the rounder corners so I decided to keep it
; using the 255 bytes.
;
; The GameStartTuneTable is merged with the PF0 data because the lower nybbles
; aren't used for PF0 graphic data. Merging the data saves 32 bytes.
;
...
; permission.
;
; NOTE: These are placed after the maze data to make zero start at the beginning
; of a page boundary. This is so the suppressZero subroutine only needs to check
; for zero to know if the zero value is to be suppressed.
;
NumberFonts
zero
   .byte $00 ; |........|
...
;
; Determine fruit index based on current game level. Kurt removed the table that
; was used for this calculation during the first iteration. Doing so makes the 
; routine take longer to execute but it saves ROM space. I'm not concerned with 
; the cycles that get used because it's not called during time sensitive 
; situations. Thanks Kurt!
;
SetFruitIndexForLevel
...
;------------------------------------------------------------------RemoveObjects
;
; Remove all objects from the screen by placing them out of range (i.e. above
; the start of the kernel).
;
RemoveObjects
   ldx #MAX_NUM_OBJ - 1
   lda #H_KERNEL + H_OBJECTS        ; place sprite out of range
...
;---------------------------------------------------------------------GameKernel
;
; This is the main kernel. This kernel does a constant 20Hz flicker on the
; objects. Doing a variable flicker system would have expanded this game over 
; the 4K limit I'm shooting for.
;
; ------------------ Maze PF Timings ------------------
...
;--------------------------------------   dot kernel
   ...
   ldx kernelSection          ; 3
   lda mazeDots + 20,x        ; 4         get the right dot values for section
   SLEEP 2                    ; 2
...
.clear
   ...
   pha                              ; clears data and moves stack pointer
   bne .clear                       ; continue until x reaches 0 (255 times)

;-------------------------------------------------------------GameInitialization
;
; Initialize the game variables on cart start up. These variables must be set
...
PinkyInterestedOffsetValues
   .byte 0 - 1, PINKY_INTERESTED_OFFSET_VALUE - 1
   .byte 0 - 1, -PINKY_INTERESTED_OFFSET_VALUE - 1
;
...
;-----------------------------------------------------------------------Overscan
;
; The display kernel is done at this point. So we turn off the TIA and do more
...
; energizer timer is reduced every other frame and is avoided when Pac-man is
; paused for eating a monster (i.e. while monster points are shown).
;
CheckEnergizerValues
...
.doneDeterminePacmanNewDirectionForDemo
   ...
   bmi .doneDeterminePacmanNewDirection
.startNewGame
...
;---------------------------------------------------------------------MovePacman
;
; Determine if it's time to move Pac-man. Decrement Pac-man's motion delay until
; an overflow occurs.
;
MovePacman
...
CollisionCheck SUBROUTINE
   ...
   bpl .doneCollisionCheck          ; skip if Pac-man eating a monster
...
; increment the score accordingly. We have to take the energizers in account
; here too. Their *real* value is not stored in the dot array like the rest of 
; the dots. The energizer blink state is stored in the dot array because it's
; used by the kernel for drawing purposes. The real energizer values are stored
; in D5 - D2 of pacmanAttributes.
;
CheckForEatingDots
...
;-------------------------------------------------------------CheckToEnableFruit
;
; The fruit is shown twice during a round and is based on the number of dots
; Pac-man has eaten. The number of dots in my game is less than the number of
; dots in the arcade version.
;
; The first fruit is shown after 44 dots have been eaten (i.e. ~29%). The second
; fruit is shown after 107 dots have been eaten (i.e. ~69%).
;
CheckToEnableFruit
   lda fruitTimer
   ...
   lda #FIRST_FRUIT_SHOWN           ; set status show first fruit has been shown
...
.checkToTurnOnSecondFruit
   ...
   lda #SECOND_FRUIT_SHOWN          ; set for second fruit has been shown
.setFruitShownState
...
;--------------------------------------------------------------------BCDToDigits
;
; Convert the score into game fonts. The leading zeros are suppressed so they
; are not shown.
;
BCDToDigits
   ldy #2
...
; starts a new television frame. Each frame starts with 3 vertical sync lines.
; These signal to to the television to start a new frame.
;
...
;-------------------------------------------------------------------MoveMonsters
;
; Loop through each monster to see if it's time to be moved. This routine also
; changes the monster direction if they're at an intersection.
;
MoveMonsters
   ldx #NUM_MONSTERS - 1
.moveMonsterLoop
...
.movePlayers
   sbc #15                    ; 2       divide player's horizontal position by 15
   bcs .movePlayers           ; 2³
...
;
; The first check is determined whether the monster is the pen or if the
; monster's eyes are returning to the pen. The next step is to check if the
; monsters are interested in finding Pac-man or the respective home corners.
;
; The targeting system for the objects are the same. They all will first try to
; reduce the greater distance (horizontal or vertical) between themselves and
; their target.
;
; Even though the targeting systems are the same; each monster has a unique way
; of finding Pac-man if they are interested
;
; Blinky (i.e. Shadow or Red)
; ===========================
; Blinky is the most aggressive of all the monsters. He will target Pac-man's
; actual horizontal and vertical position.
;
; Pinky (i.e. Speedy or Pink)
; ===========================
; Pinky will try to ambush Pac-man by targeting four tiles in front of the
; direction Pac-man is traveling.
;
; Inky (i.e. Bashful or Blue)
; ===========================
; Inky's target is based on Blinky's position to Pac-man. His targeting is also
; based on Pinky's in a way. First he tries to ambush Pac-man by targeting two
; tiles in front of the direction Pac-man is traveling. Then this value is
; reduced by Blinky's position to get Inky's target position.
;
; Clyde (i.e. Pokey or Brown)
; ===========================
; Clyde takes on Blinky's behavior where he targets Pac-man's actual position.
; Once Clyde gets within eight tiles of Pac-man then he targets his home corner.
;
MonsterAI
...
;
; Now determine which direction the monster should travel now that the desired
; target values and allowed directions have been determined.
;
; The monster's new direction is based on reducing the greatest distance from
; itself and it's respective target. If the vertical and horizontal distances
; are the same then the direction is chosen based on a direction priority. The
; priority order from lowest to highest priority is...RIGHT, DOWN, LEFT, UP
;
; Thanks Thomas for simplifying my original overly complicated first attempt.
; This saves ROM and is much easier to follow.
;
DetermineMonsterNewDirection
...
; The values are divided by 2 so they are updated every other frame. Also notice
...
;
; Enter this routine with x set to the object id you are trying to move. When
; this routine exits, allowedMotion will hold the allowed directions for the
; maze section of the object.
;
; My first attempt was to use a table to look for the object section. This took
; less ROM but could potentially run for 108 cycles.
;
DetermineAllowedMotion
